...
void ide_read(u_int diskno, u_int secno, void *dst, u_int nsecs) {
	...
	for (u_int off = 0; begin + off < end; off += BY2SECT) {
		...
		u_int temp_off = begin + off;
		panic_on(syscall_write_dev(&temp_off, DEV_DISK_ADDRESS + DEV_DISK_OFFSET, 4));
		u_int temp_op = DEV_DISK_OPERATION_READ;
		panic_on(syscall_write_dev(&temp_op, DEV_DISK_ADDRESS + DEV_DISK_START_OPERATION, 4));
		u_int temp_status;
		panic_on(syscall_read_dev(&temp_status, DEV_DISK_ADDRESS + DEV_DISK_STATUS, 4));
		panic_on(temp_status == 0);
...
void ide_write(u_int diskno, u_int secno, void *src, u_int nsecs) {
	...
	for (u_int off = 0; begin + off < end; off += BY2SECT) {
		...
		panic_on(syscall_write_dev(&temp, DEV_DISK_ADDRESS + DEV_DISK_ID, 4));
                u_int temp_off = begin + off;
                panic_on(syscall_write_dev(&temp_off, DEV_DISK_ADDRESS + DEV_DISK_OFFSET, 4));
                u_int temp_op = DEV_DISK_OPERATION_WRITE;
                panic_on(syscall_write_dev(&temp_op, DEV_DISK_ADDRESS + DEV_DISK_START_OPERATION, 4));
                u_int temp_status;
                panic_on(syscall_read_dev(&temp_status, DEV_DISK_ADDRESS + DEV_DISK_STATUS, 4));
                panic_on(temp_status == 0);
