...
static void __attribute__((noreturn)) cow_entry(struct Trapframe *tf) {
	...
	if (!(perm & PTE_COW)) {
		user_panic("cow_entry: va's page is not a copy-on-write page");
	}
	...
	u_int rva = ROUNDDOWN(va, BY2PG);
	memcpy((void *)UCOW, (void *)rva, BY2PG);

	// Step 5: Map the page at 'UCOW' to 'va' with the new 'perm'.
	/* Exercise 4.13: Your code here. (5/6) */
	panic_on(syscall_mem_map(0, (void *)UCOW, 0, (void *)rva, perm));
...
static void duppage(u_int envid, u_int vpn) {
	...
	if ((perm & PTE_D) && !(perm & PTE_LIBRARY) && !(perm & PTE_COW)) {
		...
		panic_on(syscall_mem_map(envid, (void *)addr, 0, (void *)addr, perm));
	} else {
		panic_on(syscall_mem_map(0, (void *)addr, envid, (void *)addr, perm));
	}
}
