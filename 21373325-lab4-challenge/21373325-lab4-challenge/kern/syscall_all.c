...
int sys_sigaction(int signum, const struct sigaction *act, struct sigaction *oldact) {
	...
	if (oldact != NULL) {
		*oldact = curenv->env_sigaction[signum - 1];
	}
	curenv->env_sigaction[signum - 1] = *act;
	return 0;
}

int sys_sigprocmask(int how, const sigset_t *set, sigset_t *oldset) {
	if (oldset != NULL) {
		*oldset = curenv->env_sig_mask;
	}
	if (how == SIG_BLOCK) {
		curenv->env_sig_mask.sig[0] |= set->sig[0];
        	curenv->env_sig_mask.sig[1] |= set->sig[1];
	} else if (how == SIG_UNBLOCK) {
		curenv->env_sig_mask.sig[0] &= ~(set->sig[0]);
		curenv->env_sig_mask.sig[1] &= ~(set->sig[1]);
	} else if (how == SIG_SETMASK) {
		curenv->env_sig_mask = *set;
	} else {
		return -E_UNSPECIFIED;
	}
	return 0;
...
int sys_kill(u_int envid, int sig) {
	...
	s->signum = sig;
    	//s->sigstate = SIG_PENDING;
	LIST_INSERT_HEAD(&e->env_sig_pending, s, sig_link);
	return 0;
}

void sys_set_sig_entry(void *addr) {
	curenv->env_sig_entry = (u_int)addr;
	//printk("sig entry set\n");
}

void sys_sig_finish(void) {
	curenv->env_sig_now = 0;
}
...
void do_syscall(struct Trapframe *tf) {
	...
	//memcpy(&arg4, (void *)(tf->regs[29] + 16), 4);
	//memcpy(&arg5, (void *)(tf->regs[29] + 20), 4);
